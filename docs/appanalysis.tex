...
\todo{:what does \<@RequiredPermissions> mean?  Why do we care?  What
...
class TelephonyManager {
    public @Sources(SPARTA\_Permission.PHONE_NUMBER) String getLine1Number();
    public @Sources(SPARTA\_Permission.IMEI) String getDeviceId();
}
\end{Verbatim}
...
\begin{alltt}
    ...
    @Sinks(NETWORK) String url = "http://bazinga.com/";
\end{alltt}

Without warning suppression the assignment raises an error, because string literals
are assumed to be annotated with \<Sources(LITERAL)>. By adding the suppression, you
...
%  LocalWords:  PolySources PloySinks SPARTA\_Permission SPARTA\_Permission grep
