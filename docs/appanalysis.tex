...
class TelephonyManager {
    public @Sources(SPARTA_Permission.PHONE_NUMBER) String getLine1Number();
    public @Sources(SPARTA_Permission.IMEI) String getDeviceId();
}
\end{Verbatim}
...
\begin{alltt}
    @Sources(SPARTA_Permission.CAMERA)
    @Sinks(SPARTA_Permission.NETWORK) Picture data;
\end{alltt}
...
\begin{alltt}
    ...
    @Sinks(SPARTA_Permission.NETWORK) String url = "http://bazinga.com/";
\end{alltt}

Without warning suppression the assignment raises an error, because string literals
are assumed to be annotated with \<Sources(SPARTA_Permission.LITERAL)>. By adding the suppression, you
...
%  LocalWords:  RequiredPermissions Sources Sinks apiusage Dstubs
%  LocalWords:  flowstubfiles TelephonyManager getLine1Number flowshow url
%  LocalWords:  PolySources PloySinks SPARTA_Permission SPARTA_Permission grep
