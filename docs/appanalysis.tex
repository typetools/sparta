...
\item Compare the used permissions with the description of the
  ...
  (Determine where these permissions are used in the application,
...
locations. This target only reports about suspicious APIs that appear 
in the file:
...
In addition to the reportsuspicious target, \<sparta-code/suspicious.pl> 
can be used to recursively search for suspicious String patterns in
\verb|java| and \verb|xml| files.
...
\<reportusage> target only reports about APIs that appear in the file:
\<sparta-code/src/sparta/checkers/report.astub>. This report is useful
for program comprehension but not intended to provide suspicious code
locations. For suspicious API use, see \ref{sec:appanalysis/suspicious}
%TODO: I think this belongs in the next section
%Additional \<.astub> files can be passed as \<-Astubs=...> argument in the
%build file.

The following example from \<report.astub> causes the reportusage 
...
The \<report.astub> file should be enhanced while and after the 
