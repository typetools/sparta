...
Generating a component map file for an app is a precursor to type-check its
information flow types across communicating components and within components.
In order to
generate a component map file it is necessary to create a folder containing the
target app's .apk file and all .apk files from the apps that may
communicate with the target app. After creating this folder, run the
\<generate-filters> script located in the \<sparta-code> folder as follows:
 \begin{alltt}
 generate-filters   \textit{path to a folder containing .apk files}
 \end{alltt}

After running the \<generate-filters> script, run the \<generate-cm> script,
which is located in the \<sparta-code> folder.
It receives as arguments the path of the application's .apk and the output
path of the component map file. The second one is optional and the component
map file will be output to a file named component-map in the same folder as the
.apk if it is not specified. The .apk of the app must be built without
annotations by running \<ant debug> in the app's folder.
\begin{alltt}
generate-cm   \textit{path to app's .apk}   [component map path]
\end{alltt}
...
their .java files in the \<sparta-code/annotated-intent-receivers> folder.
...
\theFlowChecker{} in the command line use the command \<check-intent> instead
of \<check-flow>:
...
This warning occurs when the receiver's \<@IntentMap> has one \<@Extra>
annotation with key missing from the sender's \<@IntentMap>.

\begin{Verbatim}
tests/intent/ActivityTest.java:128: error: [send.intent.missing.key] 
            startActivity(senderIntent1);
                          ^
The receiver's @IntentMap has an @Extra with key string value "k5", which is
missing in the sender's @IntentMap.
Sender's intent type: @sparta.checkers.quals.IntentMap({@sparta.checkers.quals.
Extra(key="k2", source={sparta.checkers.quals.FlowPermission.ACCESS_FINE_LOCATION},
sink={})}). 
Receiver's intent type: @sparta.checkers.quals.IntentMap({@sparta.checkers.quals.
Extra(key="k5", source={sparta.checkers.quals.FlowPermission.ACCESS_FINE_LOCATION},
sink={})}).
\end{Verbatim}

To solve this warning you need to add an \<@Extra> with key \<"k5"> in the
sender's \<@IntentMap>.

\subsubsection{Warning: send.intent.incompatible.types}
This warning occurs when the receiver's \<@IntentMap> has one \<@Extra> annotation
whose \<@Source> and/or \<@Sink> type are not a subtype of the receiver's extra with
same key.

\begin{Verbatim}
tests/intent/ActivityTest.java:130: error: [send.intent.incompatible.types] 
            startActivity(senderIntent2);
                          ^
The sender's @IntentMap has an @Extra annotation with key string value "k5",
with @Source and/or @Sink types which are not a subtype of the receiver's @Extra
with same key. 
Sender's intent type: @sparta.checkers.quals.IntentMap({@sparta.checkers.quals.
Extra(key="k5", source={sparta.checkers.quals.FlowPermission.ANY}, sink={})}). 
Receiver's intent type: @sparta.checkers.quals.IntentMap({@sparta.checkers.quals.
Extra(key="k5", source={sparta.checkers.quals.FlowPermission.ACCESS_FINE_LOCATION},
sink={})}).
...
To solve this warning you need to modify the \<@Source> type of the
\<@Extra> with key \<"k5"> in the sender's \<@IntentMap> to \<ACCESS\_FINE\_LOCATION>.
...
To solve this warning you need to replace \<VALUE\_KEY> with a compile time
constant value.

\subsubsection{Warning: argument.type.incompatible}
This warning occurs when an element is inserted into an intent's map but it
does not have its expected \<@Source> and/or \<@Sink> type according the
...
To solve this warning you need to modify the \<@Extra> with
