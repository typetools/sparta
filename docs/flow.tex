...
\<sparta.checkers.quals.SPARTA_Permission> and the sink must be a
member of the enum 
\<sparta.checkers.quals.SPARTA_Permission>.  The source 
...
%given policy file.  Fake SPARTA_Permission and SPARTA_Permission names are used
%to aid in clarity.
%
...
%% Adding on to that, if you try to compute a @SPARTA_Permission{something} int
%%  with a int literal (0 or 100, etc) it will resolve to another
%%  @SPARTA_Permission{something} int. If you try the same with a
%%  @SPARTA_Permission{something} int with an int literal, it will result in
%%  @SPARTA_Permission{} int because it's not safe to add a flowsink to the int
%%  literal. To bypass this, you must declare a local variable with
%%  "@SuppressWarnings("flow") @SPARTA_Permission{something} int tempZero = 0;"
