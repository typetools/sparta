...
see \secref{sec:system:defaults}.  It is also
...
\myparagraph{Empty Sink or Source\label{sec:emptyflow}}
...
annotations. 
% The type's flow sinks
% depend on where literals are permitted to flow according to the policy file.

%\todo{@Sink(CONDITIONAL) is not a hole:  uses at method call sites must be consistent.  %Good for computation.}  No idea what the write of this todo wants
...
Any method not written in the stub files or found in source code is not defaulted normally. 
 ...
 in a policy file and in no subtype relation.  This way, if such an API method is used, a type error
  will occur and alert the user to review and annotate the method. Althought it is possible to ignore these kind of warnings as explained in section \secref{sec:run-type-checker}.
