...
%given policy file.  Fake SPARTA\_Permission and SPARTA\_Permission names are used
%to aid in clarity.
%
...
%% Adding on to that, if you try to compute a @SPARTA\_Permission{something} int
%%  with a int literal (0 or 100, etc) it will resolve to another
%%  @SPARTA\_Permission{something} int. If you try the same with a
%%  @SPARTA\_Permission{something} int with an int literal, it will result in
%%  @SPARTA\_Permission{} int because it's not safe to add a flowsink to the int
%%  literal. To bypass this, you must declare a local variable with
%%  "@SuppressWarnings("flow") @SPARTA\_Permission{something} int tempZero = 0;"
