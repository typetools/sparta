...
%given policy file.  Fake SpartaPermission and SpartaPermission names are used
%to aid in clarity.
%
...
%% Adding on to that, if you try to compute a @SpartaPermission{something} int
%%  with a int literal (0 or 100, etc) it will resolve to another
%%  @SpartaPermission{something} int. If you try the same with a
%%  @SpartaPermission{something} int with an int literal, it will result in
%%  @SpartaPermission{} int because it's not safe to add a flowsink to the int
%%  literal. To bypass this, you must declare a local variable with
%%  "@SuppressWarnings("flow") @SpartaPermission{something} int tempZero = 0;"
