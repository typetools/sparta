...
TheFlowChecker's permissions are finer-grained than standard Android manifest permissions
...
%\todo{Should we mention something about the source/sinks that 
%SuSi found here or in the related works?}
%The related work in the appropriate place.  
%SuSi doesn't not claim to find all the categories of sensitive sources/sinks
% or even all the methods that access resources. 
%Our system does not add much complexity: 
%it only adds \numIFCperms permissions to
%Android's standard \numandroidperms, or \percentIFCperms more permissions.  


Some researchers feel that the Android permission model is already too complicated for
users to understand~\cite{FeltCHSW2011}.  But our perspective is that of a
full-time auditor who
is trained to analyze applications.  The flow policy is examined once
per application by that skilled engineer, not on every download by a user, so
the total human burden is less.  (See
\secref{sec:empirical-study:appstore} for empirical measurements.)
  The more detailed flow
policy file yields more insight than simple Android permissions,
because the flow policy (\secref{sec:system:policy})
makes clear how each resource is used, not just
that it is used.
...
%Each finer grained annotation also has a params attribute, which is a
