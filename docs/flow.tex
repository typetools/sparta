...
troubleshooting). \todo{Shouldn't this be moved closer to the top of the section?}
...
\begin{Verbatim}
  @Sources(FlowSource.LOCATION) @Sinks(FlowSink.NETWORK) double latitude;
\end{Verbatim}
\noindent
The \<@Sources(FlowSource.LOCATION)> annotation indicates that the
...
\figref{fig:flow-hierarchy} shows part of the \<@Sinks>
...
\begin{alltt}
@Sources(FlowSource.USER_INPUT) @Sinks(FlowSink.FILESYSTEM)
...
\begin{alltt}
  @Sources(FlowSource.ACCOUNTS) @Sinks(FlowSink.EXTERNAL_STORAGE) Account acc = ...
  @Sources({FlowSource.ACCELEROMETER, FlowSource.ACCOUNTS})
  @Sinks({FlowSink.EXTERNAL_STORAGE, FlowSink.FILE_SYSTEM}) int accel = ...
\end{alltt}

The following definitions would generate ``forbidden flow'' errors:
\begin{Verbatim}
  @Sources(FlowSource.ACCOUNTS) @Sinks(@FlowSink.NETWORK) Account acc = ...
  @Sources({FlowSource.ACCELEROMETER, FlowSource.ACCOUNTS})
  @Sinks({FlowSink.EXTERNAL_STORAGE, FlowSink.FILESYSTEM, FlowSink.NETWORK})
\end{Verbatim}
...
\<sparta.checkers.quals.Sources.FlowSource> and the sink must be a
member of the enum 
\<sparta.checkers.quals.Sinks.FlowSink>.  The source 
...
allowing any flows.
\todo{is this right? What about relaxed vs. strict?}
...
\midrule 
    Fields &  \<@Sources(FlowSource.LITERAL)> \\ 
    Method parameters &  \<@Sources(FlowSource.LITERAL)> \\ 
    Return values &  \<@Sources(FlowSource.LITERAL)> \\ 
    \<null> &  \<@Sources(\{\}) @Sinks(FlowSink.ANY)>\\ 
    Literals & \<@Sources(FlowSource.LITERAL) @Sinks(FlowSink.CONDITIONAL)>\\ 
   Local variables  &   \<@Sources(FlowSource.ANY) @Sinks(\{\})> \\ 
...
rather than  \<@Sources({FlowSource.LITERAL})>. Note that \<@Sources>
means \<@Sources(\{\})> and \<@Sinks>  means \<@Sinks(\{\})>.
\begin{alltt}
public void sendToInternet(
    @Sources(FlowSource.LOCATION) @Sinks(FlowSink.NETWORK) String message)\{...\}
\end{alltt} 

The value \<null> has the bottom type (\<@Sinks(FlowSink.ANY)
@Sources>),
...
method's return type and all parameter types are \<@Sources(FlowSource.LITERAL)
@Sinks(FlowSink.CONDITIONAL)>.  As with all 
...
\begin{alltt}
[jsr308.javac] ... warning: "@Sinks @Sources({FlowSource.ANY}) Object"
...
possible return type, \<@Sources(FlowSource.ANY) @Sinks({})>.
...
%  LocalWords:  Sinks someData Sources enum 5cm flowsources 'c'
%  LocalWords:  flowsinks realString asdf PolySources PolySinks
