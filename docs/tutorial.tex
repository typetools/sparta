...
\todo{What if instead of forbiddenFlows.txt, the Information Flow Checker output a draft-flow-policy,
...
one by one; correcting it and then re-running the Information Flow Checker.  In general, 
...
Run the Information Flow Checker again and start with the first warning:
...
\item\textbf{Where do the found and required types come from?}
  ...
  \<@Source(CONTENT\_PROVIDER>.\newline
    ...
    Run the Information Flow Checker; there should be no warnings!  This means that the only information 
