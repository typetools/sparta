...
LinkedHashSet
       /* Because @Source(LITERAL) is not a subtype of @Source(CAMERA), assigning getBAOSLinkedHashSet
          to bytes produces an assignment warning. (The waring is supressed.)
          @Sink( DISPLAY,WRITE_LOGS, FILESYSTEM) is a subtype of
          @Sink({ DISPLAY, FILESYSTEM}), so the sink of bytes is refined toLinkedHashSet
          @Sink( DISPLAY,WRITE_LOGS, FILESYSTEM). The full type of bytes is
          @Source(CAMERA) @Sink({ DISPLAY,WRITE_LOGS, FILESYSTEM}), butLinkedHashSet
          this is not valid type because CAMERA->WRITE_LOGS is a forbidden flow.LinkedHashSet
